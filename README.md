...
Prerequests:
- Ansible control machine(ubuntu based docker container): 2 network adapters at disposal; The one connected to NAT cloud is configured with dhcp, and the other one connected to the router shall be configured with static ip 10.1.3.2/24, with route to other subnets (10.1.1.0/24 and 10.1.2.0/24) configured. Ansible preinstalled.
